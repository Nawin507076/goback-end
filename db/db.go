...
func NewDB() (*DB, error) {
	url := "host=localhost user=peanawin password=supersecret dbname=peagolang port=40000 sslmode=disable"
...
func (db *DB) CreateClass(cls *model.Class) error {
	class := Class{
...
func (db *DB) GetUserByID(id uint) (*model.User, error) {
	var user User
	err := db.db.First(&user, id).First(&user).Error
	if err != nil {
		return nil, err
	}
	return &model.User{
